...
/// REST request body for `POST /api/v1/tasks` — create a single cracking task.
...
/// Attack configuration for one campaign phase. The serialized `type` tag
/// (snake_case) selects the variant.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(tag = "type", rename_all = "snake_case")]
pub enum PhaseConfig {
    /// A single fixed mask attack.
    StaticMask {
        mask: String,
        custom_charsets: Option<Vec<String>>,
    },
    /// A sequence of fixed masks, run in order.
    MultiMask { masks: Vec<MaskEntry> },
    /// Masks derived by the pattern analyzer from already-cracked plaintexts
    /// once `min_sample_size` is reached (capped at `max_masks`).
    AutoGenerated {
        min_sample_size: u32,
        max_masks: u32,
    },
    /// Brute force over `charset` for every length in `min_length..=max_length`.
    ExpandingBrute {
        charset: String,
        min_length: u32,
        max_length: u32,
    ...
    /// Dictionary attack with an optional list of rule strings.
    Dictionary {
        wordlist_file_id: String,
        rules: Vec<String>,
    },
    /// Hybrid wordlist+mask attack (hashcat attack `mode`).
    Hybrid {
        wordlist_file_id: String,
        mask: String,
        mode: u32,
...
/// REST request body for `POST /api/v1/campaigns` — create a multi-phase
/// campaign from inline `phases` or a named `template`.
#[derive(Debug, Deserialize)]
