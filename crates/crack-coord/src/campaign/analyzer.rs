//! Pattern analyzer: derives new hashcat masks from already-cracked plaintexts
//! for `AutoGenerated` campaign phases. Builds character-class skeletons, ranks
//! them by frequency, and emits masks bounded by a configured max keyspace.
...
/// Tunables for [`analyze`]: sample thresholds, how many skeletons to consider,
/// charset-coverage cutoff, the max keyspace a generated mask may have, and the
/// cap on masks returned.
...
/// Output of [`analyze`]: ranked candidate masks plus a summary.
#[derive(Debug, Clone)]
...
/// One candidate mask with its score, how it was derived, and estimated
/// keyspace. (Some fields are informational and not read by every caller.)
#[derive(Debug, Clone)]
#[allow(dead_code)]
...
/// How a [`GeneratedMask`] was derived.
#[derive(Debug, Clone)]
pub enum MaskSource {
    /// Direct character-class skeleton of observed plaintexts.
    DirectSkeleton,
    /// Skeleton narrowed with a per-position custom charset.
    CustomCharset,
    /// Skeleton with a length extended/shrunk by ±1.
    LengthVariant,
    /// Mask anchored on a frequently-observed suffix.
    SuffixAnchored,
}

/// Summary counters for an [`AnalysisResult`].
#[derive(Debug, Clone)]
#[allow(dead_code)]
...
impl Skeleton {
    ...
    /// Total keyspace of this skeleton's mask, or `None` if it overflows `u64`.
    ///
    /// `checked_mul` (vs the old `saturating_mul`) makes overflow explicit: a
    /// saturated `u64::MAX` would compare `> max_keyspace` and be skipped
    /// anyway, but it also makes two genuinely-distinct astronomical keyspaces
    /// indistinguishable. `None` cleanly means "too large to be a useful mask".
    fn keyspace(&self) -> Option<u64> {
        let mut ks: u64 = 1;
        for ch in self.0.chars() {
...
/// Derive ranked candidate masks from already-cracked `passwords`, skipping any
/// mask in `already_run` and bounding results by `config`. Returns the masks
/// ordered best-first plus summary counters.
pub fn analyze(
    ...
    for (skel, count) in &ranked {
        ...
        let Some(ks) = skel.keyspace() else {
            continue;
        };
    ...
    for (skel, _count) in ranked.iter().take(20) {
        ...
        if has_narrowing
            ...
            && custom_ks > 0
        {
            let base_skel_ks = skel.keyspace().unwrap_or(0);
    ...
    for (skel, count) in ranked.iter().take(10) {
        ...
        for delta in [-1i32, 1, 2] {
            if let Some(variant) = length_variant(skel, delta) {
                ...
                let Some(ks) = variant.keyspace() else {
                    continue;
                };
                if ks > config.max_keyspace || ks == 0 {
                    continue;
...
mod tests {
    ...
    fn test_skeleton_keyspace() {
        ...
        assert_eq!(skel.keyspace(), Some(26 * 26 * 10));

        // A mask long enough to overflow u64 reports None rather than a
        // saturated (and ambiguous) u64::MAX.
        let huge = Skeleton("A".repeat(20));
        assert_eq!(huge.keyspace(), None);
    }
