...
pub async fn start_campaign(state: &Arc<AppState>, campaign_id: Uuid) -> Result<()> {
    ...
    if campaign.status != CampaignStatus::Draft {
        anyhow::bail!("campaign {} is not in draft status (current: {})", campaign_id, campaign.status);
    }
    ...
    db::insert_audit(
        &state.db,
        "campaign_started",
        &format!("Campaign '{}' started with {} hashes, {} phases", campaign.name, total_hashes, campaign.total_phases),
        None,
        None,
    )
    .await?;
    ...
    if task.status != TaskStatus::Failed {
        if let Some(next) = find_next_subtask(state, &campaign, &phase, &task).await? {
            ...
            let new_task = create_phase_task(
                state, &campaign, &phase, &hash_file_id,
                &next.mask, next.custom_charsets, &next.name,
            ).await?;
    ...
    match &phase.config {
        PhaseConfig::MultiMask { masks } => {
            ...
            Ok(Some(NextSubTask {
                ...
                name: format!(
                    "{} — {} [{}/{}]",
                    campaign.name, phase.name, next_idx + 1, masks.len()
                ),
            }))
        }

        PhaseConfig::ExpandingBrute { charset, min_length, max_length, custom_charsets } => {
            ...
            Ok(Some(NextSubTask {
                ...
                name: format!(
                    "{} — {} [len={}]",
                    campaign.name, phase.name, next_length
                ),
            }))
        }

        PhaseConfig::AutoGenerated { min_sample_size, max_masks } => {
...
async fn advance_phase_inner(state: &Arc<AppState>, campaign_id: Uuid) -> Result<()> {
    ...
    let next_phase = phases
        ...
        .ok_or_else(|| anyhow::anyhow!("phase {next_index} not found for campaign {campaign_id}"))?;

    db::advance_campaign_phase(&state.db, campaign_id, next_index).await?;

    // Create filtered hash file
    let filtered_file_id = create_filtered_hash_file(state, campaign_id, &campaign.original_hash_file_id).await?;
...
async fn start_phase_inner(
    ...
    match &phase.config {
        PhaseConfig::StaticMask { mask, custom_charsets } => {
            let task = create_phase_task(
                state, campaign, phase, hash_file_id,
                mask, custom_charsets.clone(),
        ...
        PhaseConfig::MultiMask { masks } => {
            if let Some(entry) = masks.first() {
                let task = create_phase_task(
                    state, campaign, phase, hash_file_id,
                    &entry.mask, entry.custom_charsets.clone(),
        ...
        PhaseConfig::AutoGenerated { min_sample_size, max_masks } => {
            ...
            if result.masks.is_empty() {
                ...
                db::update_phase_status(&state.db, phase.id, PhaseStatus::Skipped).await?;
                db::insert_audit(&state.db, "campaign_phase_skipped",
                    &format!("Phase {} '{}' skipped: no new masks ({} passwords, {} skeletons)",
                        phase.phase_index, phase.name,
                        result.summary.total_passwords, result.summary.unique_skeletons),
                    None, None).await?;
                advance_phase(state, campaign.id).await?;
                return Ok(());
            }

            db::insert_audit(&state.db, "campaign_auto_masks",
                &format!("Phase {} '{}' generated {} masks from {} passwords ({} skeletons)",
                    phase.phase_index, phase.name, result.masks.len(),
                    result.summary.total_passwords, result.summary.unique_skeletons),
                None, None).await?;
            ...
            if let Some(gm) = result.masks.first() {
                let task = create_phase_task(
                    state, campaign, phase, hash_file_id,
                    &gm.mask, gm.custom_charsets.clone(),
                    &format!("{} — {} [auto 1/{}]", campaign.name, phase.name, result.masks.len()),
                ).await?;
                db::set_phase_task_id(&state.db, phase.id, task.id).await?;
            }
        }

        PhaseConfig::ExpandingBrute { charset, min_length, max_length: _, custom_charsets } => {
...
async fn create_filtered_hash_file(
    ...
    let (file_id, sha256) = files::save_file(
        &state.files_dir(), &filename, filtered_content.as_bytes(),
    )?;
...
fn collect_masks_from_config(config: &PhaseConfig, set: &mut HashSet<String>) {
    match config {
        PhaseConfig::StaticMask { mask, .. } => { set.insert(mask.clone()); }
        PhaseConfig::MultiMask { masks } => {
            for entry in masks { set.insert(entry.mask.clone()); }
        }
        PhaseConfig::ExpandingBrute { charset, min_length, max_length, .. } => {
            for len in *min_length..=*max_length { set.insert(charset.repeat(len as usize)); }
        }
        _ => {}
    }
}
