...
async fn find_next_subtask(
    ...
    match &phase.config {
        ...
        PhaseConfig::AutoGenerated {
            ...
            // Masks already run across all phases + existing brute-force tasks.
            let already_run = collect_already_run_masks(state, campaign.id).await?;
    ...
    match &phase.config {
        ...
        PhaseConfig::MultiMask { masks } => {
            if let Some(entry) = masks.first() {
                ...
                // No masks → no task would ever be created, leaving the phase
                // Running forever and stalling the campaign. Skip and advance,
                // mirroring the empty-sample AutoGenerated path.
                info!(campaign_id = %campaign.id, phase = phase.phase_index,
                    "MultiMask phase has no masks, skipping");
...
async fn create_filtered_hash_file(
    ...
    // Read off the async runtime: the hash file can be large and read_file is
    // blocking std::fs.
    let files_dir = state.files_dir();
    let orig = original_file_id.to_string();
    let file_data = tokio::task::spawn_blocking(move || files::read_file(&files_dir, &orig))
        .await
        .context("joining hash-file read task")?
    ...
    // Write off the async runtime (blocking std::fs).
    let files_dir = state.files_dir();
    let fname = filename.clone();
    let bytes = filtered_content.into_bytes();
    let (file_id, sha256, size_bytes) =
        tokio::task::spawn_blocking(move || files::save_file(&files_dir, &fname, &bytes))
            .await
            .context("joining filtered-file save task")?
            .context("saving filtered hash file")?;
...
/// Every mask already run for a campaign — across all phase configs and all
/// existing brute-force tasks — so the analyzer never regenerates one.
///
/// Previously this logic was copy-pasted into both `find_next_subtask` and
/// `start_phase_inner` for the `AutoGenerated` path; the duplication meant the
/// two scans could drift. Consolidated here.
///
/// # Errors
/// Returns an error if either underlying campaign query fails.
async fn collect_already_run_masks(
    state: &Arc<AppState>,
    campaign_id: Uuid,
) -> Result<HashSet<String>> {
    let mut already_run = HashSet::new();
    let all_phases = db::get_phases_for_campaign(&state.db, campaign_id).await?;
    for p in &all_phases {
        collect_masks_from_config(&p.config, &mut already_run);
    }
    let campaign_tasks = db::get_tasks_for_campaign(&state.db, campaign_id).await?;
...
pub async fn check_campaign_progress(state: &Arc<AppState>) -> Result<()> {
    // Refresh every running campaign's cracked count in one UPDATE, then find
    // the campaigns whose active phase's task has gone terminal in one JOIN —
    // instead of ~4 queries per running campaign each tick.
    db::sync_all_running_campaign_cracked_counts(&state.db).await?;
    for task_id in db::running_campaigns_terminal_tasks(&state.db).await? {
